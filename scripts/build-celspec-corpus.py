...
def _build_vectors_doc(sha: str, verified: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "_doc": (
            ...
            "profile are included, and only after the single relay_cel_wasm engine "
            "reproduces the upstream golden (the legacy two-engine py-vs-ts parity "
            "axis was removed in M6: both hosts now evaluate via the SAME wasm, so "
            "byte-parity is by construction). EXCLUDED vectors are also real "
            "upstream cases; their expressions use features Relay's profile rejects "
            "(dyn / timestamp / duration) and are classified in "
            "relay-profile-filter.yaml. "
...
def _build_profile_filter(verified: list[dict[str, Any]]) -> str:
    ...
    lines.append("# sets. 'included' vectors are inside Relay's CEL profile; the single")
    lines.append("# relay_cel_wasm engine reproduces the upstream golden (enforced by")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sys.stderr.write(f"[build] verified {len(verified)} included vectors (wasm==golden)\n")
    if len(verified) < 25:
